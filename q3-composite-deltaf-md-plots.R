...
# Model: deltaf
fit_recruit <- lmer(deltaf ~ group_age*muscle + recruit_s + (1 + recruit_s | participant), data = d)
# did not converge: fit_muscle_by_recruit <- lmer(deltaf ~ group_age*muscle + muscle*recruit_s + (1 + recruit_s*muscle | participant), data = d)
fit_age_by_others <- lmer(deltaf ~ group_age*muscle + group_age*recruit_s + (1 + recruit_s | participant), data = d)

# Compare models
anova(fit_recruit, fit_age_by_others)
...
ggsave(file = "composite-figure.tiff", units="in", width = 9.5, height = 3.5, dpi = 300, compression = "lzw")
